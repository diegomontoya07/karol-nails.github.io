...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombre) || empty($correo) || empty($rol) || empty($salario) || empty($contraseña)) {
        header('Location: contratacion2.php?message=Todos los campos son obligatorios.&type=error');
        exit();
    } else {
        ...
        if ($stmt->execute()) {
            header('Location: contratacion2.php?message=Usuario registrado correctamente.&type=success');
        } else {
            header('Location: contratacion2.php?message=No se pudo registrar el usuario. Intenta de nuevo.&type=error');
        }
